...
PRI-PVEM & PRD-PT-NA & PRD-PT & PRD-NA & PT-NA & VOTOS NULOS & NO REGISTRADOS \\
   ...
   este indice. Por ahora solo agrego un int (int indexinic) para indicar a partir de que indice se 
   deben empezar a contar los votos totales (i.e. int cdvotos). V\'ease el inicializador en el 
   constructor de la clase Casilla que recibe un vector de string conteniendo los tokens de una 
   linea de archivo adaptado.
  */
  vector<string> tdl; /*tokens de la linea en el "archivo adaptado" (vease include/prueba.h)*/
                      /*DEBERA inicializarse en el constructor que recibe los tokens de una linea
                        de archivo ``adaptado''*/
  int indexinic;      /*index inicial para contar los votos. I.e. for(int i=indexinic;i<tdl.size()r;i++)*/
